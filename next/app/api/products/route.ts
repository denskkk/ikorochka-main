  ...
  try{
    ...
    if(GITHUB_TOKEN && GIST_ID){
      try{
        ...
        return new NextResponse(content, { status: 200, headers: { 'Content-Type': 'application/json', 'X-Source':'gist' } })
      }catch(err:any){
...
export async function POST(req: Request){
  try{
    ...
    try {
      ...
      return NextResponse.json({ ok: true, storage: 'gist' })
    }
  }catch(e:any){
    return NextResponse.json({ error: 'cannot_write_products', message: String(e?.message || e) }, { status: 500 })
  }
}
